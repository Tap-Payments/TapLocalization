...
public extension LocalizationManager {
    ...
    public func localizedAmountString(with amount: Decimal, currencyCode: String, amountAttributes: [NSAttributedStringKey: Any?]?, currencyAttributes: [NSAttributedStringKey: Any?]?) -> NSAttributedString {
        ...
        let mutableResultString = NSMutableAttributedString(string: amountString, attributes: amountAttributes?.nonOptionalRepresentation)
        if currencyRange.location != NSNotFound && currencyRange.length > 0 {

            mutableResultString.setAttributes(currencyAttributes?.nonOptionalRepresentation, range: currencyRange)
        }
